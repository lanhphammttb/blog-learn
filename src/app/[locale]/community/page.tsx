          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            ...
            <div className="order-1 md:order-2">
               <div className="relative rounded-3xl bg-blue-600 p-10 text-center text-white shadow-2xl shadow-blue-500/20 flex flex-col items-center group transition-all hover:-translate-y-2 scale-105">
                  ...
                  <div className="mt-4 text-xs font-bold uppercase tracking-widest text-blue-200">{t('ultimate_label')}</div>
               </div>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-muted/30">
                  <th className="px-8 py-5 text-sm font-bold text-muted-foreground uppercase tracking-widest w-24 text-center">{t('table.rank')}</th>
                  <th className="px-8 py-5 text-sm font-bold text-muted-foreground uppercase tracking-widest">{t('table.student')}</th>
                  <th className="px-8 py-5 text-sm font-bold text-muted-foreground uppercase tracking-widest text-center">{t('table.xp')}</th>
                  <th className="px-8 py-5 text-sm font-bold text-muted-foreground uppercase tracking-widest text-center">{t('table.lessons')}</th>
                  <th className="px-8 py-5 text-sm font-bold text-muted-foreground uppercase tracking-widest text-right">{t('table.profile')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/50">
                {loading ? (
                  ...
                  filteredUsers.map((user, index) => (
                    <tr key={user.id} className="group hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{t('learner_label')}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-8 py-6 text-right">
                        ...
                        >
                           {t('view_profile')} <ArrowUpRight className="h-3 w-3" />
                        </Link>
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan={5} className="px-8 py-20 text-center text-muted-foreground font-medium">{t('no_students')}</td>
                  </tr>
                )}
              </tbody>
            </table>
