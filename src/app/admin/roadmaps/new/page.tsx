...
export default function NewRoadmap() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.phases.length === 0) {
      alert('Vui lòng tạo ít nhất 1 Giai đoạn (Phase) để thiết kế roadmap!');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Lỗi tạo roadmap: ' + (error.error || 'Unknown error'));
      }
    } catch (error) {
       console.error(error);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (type === 'checkbox') {
      ...
      if (name === 'title' && !formData.slug) {
        ...
        setFormData((prev: any) => ({ ...prev, [name]: value, slug: slugValue }));
        return;
      }
