...
export default function NewRoadmap() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.items.length === 0) {
      alert('Please add at least one article to the roadmap.');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Failed to create roadmap: ' + (error.error || 'Unknown error'));
      }
    } catch (error) {
      console.error(error);
  ...
  const filteredArticles = articles.filter(a => 
    ...
    !formData.items.some(item => item.articleId === a._id)
  );
             ...
             {formData.items.length === 0 ? (
                ...
                <div className="space-y-3">
                   {formData.items.map((item, index) => (
                      <div key={item.articleId} className="flex items-center gap-4 bg-muted/30 p-4 rounded-xl border border-border group">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-border bg-card p-6 shadow-xl space-y-4">
             <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-muted-foreground">{articles.length} Available</span>
             </div>
             ...
             <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {fetching ? (
                   ...
                   <p className="text-center py-4 text-[10px] text-muted-foreground italic">No articles found matching your search.</p>
                )}
             </div>
          </div>
