...
export default function RoadmapCanvas({ 
  ...
  const handleProjectSubmit = async () => {
    ...
    if (!isLoggedIn) {
       showToast('Vui lòng đăng nhập để nộp bài!', 'error');
       return;
    }

    setIsSubmittingBoss(bossId);
    showToast('Đang gửi bài nộp...', 'loading');
    try {
      ...
      if (res.ok) {
        ...
        showToast('Đã gửi bài nộp thành công! Vui lòng đợi Admin duyệt.', 'success');
      } else {
  ...
  const progressPercent = totalItems > 0 ? Math.round((globalCompletedCount / totalItems) * 100) : 0;
  ...
  return (
    <div className="relative py-12">
      ...
      <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-2 md:-translate-x-1/2 bg-muted/50 rounded-full" />
      
      {/* Timeline Track Đã Nạp Màu (Hoạt động) */}
      <div 
        className="absolute left-6 md:left-1/2 top-0 w-2 md:-translate-x-1/2 bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)] rounded-full transition-all duration-[1500ms] ease-out" 
                ...
                <div className="relative flex justify-center mb-16 z-20">
                   <div className={`p-6 md:p-8 rounded-[32px] border-4 shadow-2xl w-[90%] md:w-[60%] lg:w-[50%] transition-all text-center backdrop-blur-xl ${
                      ...
                      <div className="flex items-center justify-center gap-3 mb-2">
                         ...
                         <span className="text-sm font-black uppercase tracking-widest opacity-80">Giai Đoạn {pIndex + 1}</span>
                      </div>
                          ...
                          {isLocked ? (
                            <div className={`block ${isSatellite ? 'p-4 rounded-2xl' : 'p-6 md:p-8 rounded-[32px]'} bg-card border-2 border-border shadow-md`}>
                              <div className="flex items-center justify-between mb-4">
                                ...
                                {isSatellite && <span className="text-[10px] font-bold text-muted-foreground uppercase">Phụ Trợ</span>}
                              </div>
                          ...
                          <div className={`bg-card dark:bg-zinc-950 p-6 md:p-8 rounded-[28px] h-full relative overflow-hidden ${isBossLocked && 'grayscale'}`}>
                             ...
                             <div className="flex flex-col items-center text-center relative z-10">
                                ...
                                <span className={`text-[10px] font-black uppercase tracking-widest ${isBossLocked ? 'text-muted-foreground' : isApproved ? 'text-green-500' : isPending ? 'text-orange-500' : 'text-red-500'} mb-2`}>
                                   {isBossLocked ? 'Boss Đang Khoá' : isApproved ? 'Đã Chinh Phục' : isPending ? 'Đang Chờ Duyệt' : isRejected ? 'Bị Từ Chối - Nộp Lại' : 'Boss Milestone'}
                                </span>
                                ...
                                <p className="text-sm font-medium text-muted-foreground max-w-sm">{isBossLocked ? 'Cần hoàn thành tất cả bài học trong giai đoạn này để mở khoá Boss.' : phase.project.requirements}</p>
                                
                                {isApproved ? (
                                   <div className="mt-6 flex flex-col items-center gap-2">
                                      <div className="py-2.5 px-6 rounded-xl bg-green-500 text-white font-bold text-sm flex items-center gap-2">
                                         <CheckCircle2 className="h-4 w-4" /> Tuyệt vời!
                                      </div>
                                      {submission?.feedback && <p className="text-xs text-green-600 font-medium">Feedback: {submission.feedback}</p>}
                                   </div>
                                ) : isPending ? (
                                   ...
                                   >
                                      {isBossLocked ? <><Lock className="h-4 w-4" /> Đang Khoá Boss</> : isSubmittingBoss === bossId ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Thách đấu Boss <ChevronRight className="h-4 w-4" /></>}
                                   </button>
                                )}
                             </div>
                          </div>
      ...
      {progressPercent === 100 && (
         <div className="mt-20 flex flex-col items-center">
            ...
            <h3 className="mt-6 text-3xl font-black text-foreground uppercase tracking-widest">Tuyệt Vời!</h3>
            <p className="mt-2 text-lg text-muted-foreground font-medium">Bạn đã chinh phục hoàn toàn chặng đường này.</p>
         </div>
      )}
           ...
           <div className="relative w-full max-w-lg bg-card border border-border shadow-2xl rounded-[32px] overflow-hidden animate-in fade-in zoom-in duration-300">
              <div className="p-8">
                 <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                       <h3 className="text-2xl font-black">Thách Đấu Boss</h3>
                       <p className="text-sm text-muted-foreground font-medium uppercase tracking-tight">Giai đoạn {activeBossIndex + 1}</p>
                    </div>
                 </div>

                 <div className="mb-6 sm:mb-8 p-4 sm:p-6 rounded-2xl bg-muted/50 border border-border/50">
                    <span className="text-[10px] font-black uppercase text-red-500 mb-2 block tracking-widest">Yêu cầu nhiệm vụ:</span>
                 ...
                 <div className="space-y-3 sm:space-y-4">
                    <label className="block text-[10px] sm:text-sm font-black uppercase tracking-widest text-muted-foreground">
                       Nộp bài làm (Link hoặc Nội dung):
                    </label>
                    <textarea
                       ...
                       placeholder="Dán link Drive, Youtube bải tập, hoặc tóm tắt bài làm tại đây..."
                 ...
                 <div className="mt-6 sm:mt-8 flex flex-col-reverse sm:flex-row gap-3">
                    ...
                    >
                       {isSubmittingBoss ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Nộp bài & Phá Đảo <ChevronRight className="h-5 w-5" /></>}
                    </button>
                 </div>
              </div>
           </div>
